...
if [[ "$nodetype" == M ]] 
then 
    sudo firewall-cmd --permanent --add-port={6443,2379,2380,10250,10259,10257,179,10256}/tcp 
fi 

if [[ "$nodetype" == W ]] 
then 
    sudo firewall-cmd --permanent --add-port={10250,10256,179,30000-32767,7946}/tcp 
...
sudo firewall-cmd --zone=public --add-interface=tunl+ --permanent 
...
sudo echo "172.20.0.60  srv-k8s-m1
172.20.0.61 srv-k8s-w1
172.20.0.62 srv-k8s-w2
172.20.0.30 registry.intsmm.net" | sudo tee -a /etc/hosts > /dev/null
...
#install the registry certificate to make cri-o trust it.
sudo mkdir -p /etc/containers/certs.d/registry.intsmm.net
sudo cp registry.intsmm.net.crt /etc/containers/certs.d/registry.intsmm.net

echo installed registry certificate

sleep 2

#specify pause image in crio
sudo sed -i "/^\[crio.image\]/a pause_image = \"registry.intsmm.net\/kubernetes\/pause:3.10\"" "/etc/crio/crio.conf.d/10-crio.conf"
sudo systemctl restart crio
echo added pause image to crio config
